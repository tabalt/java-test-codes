...
public class NioServer2 {
	...
	public void start(int port) {
		...
		try {
			...
			System.out.println("listen " + port + " failed, server will stop.");
			e.printStackTrace();
			return;
		}
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			try {
				...
				System.out.println("select " + port + " failed, server will stop.");
				e.printStackTrace();
				return;
			}
			...
			while (keyIterator.hasNext()) {
				...
				try {
					...
					System.out.println("accept/read/write " + port + " failed, server will continue to deal next connection.");
					e.printStackTrace();
					key.cancel();
				}
			}
